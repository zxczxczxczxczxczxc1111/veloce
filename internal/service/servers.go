...
// State отдаёт ТЕКУЩЕЕ состояние соединения.
//
// Нужен потому, что подписка на события ловит только будущее: событие
// «подключились» вполне могло прилететь до того, как экран успел открыться, и
// интерфейс, полагающийся на одни события, считал бы живой сервер отключённым
// и не запускал такты вообще.
